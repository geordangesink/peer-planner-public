...
export default ({
  ...
  const { sharedDbObject, roomIdRef, currentSchedule, setSchedule } =
    useSchedule();
  const {
    isEvent,
    setIsEvent,
  ...
  const onSave = () => {
    const newSchedule = getActivityObject(
      ...
      >
        ...
        <form className="space-y-1 space-x-1">
          <input
            ...
            ref=${titleRef}
            defaultValue=${
              editOpts.title !== '(No Title)' ? editOpts.title : ''
            }
          />
